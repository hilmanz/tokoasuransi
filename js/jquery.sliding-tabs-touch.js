(function ($) {
	...
	$.extend($.stCore, {
		...
		slideTabBack:function(speed) {
			...
			if (this.val.useWebKit) {
				...
				if (this.opt.orientation=="horizontal") {
					this.$tabs.animate({"marginLeft":this.tabs.startXY+"px"}, speed, "easeOutSine", function() {
						that.setTabIsAnim(false, "resume");
					});
				} else {
					this.$tabs.animate({"marginTop":this.tabs.startXY+"px"}, speed, "easeOutSine", function() {
						that.setTabIsAnim(false, "resume");
					});
				}
			}
		...
		slideContentBack:function(speed) {
			...
			if (this.val.useWebKit) {
				...
				this.$prevView.css({
					"-webkit-transition-duration":speed+"ms",
					"-webkit-transition-timing-function":"ease-out"
				}).css(this.content.css, this.content.pre-this.content.slideLength+b.content.px);
		...
		contentTouchDir:function(e, anim) {
			var pos = (Math.abs(e.pageX-this.content.eX)-Math.abs(e.pageY-this.content.eY))-(anim ? -5 : 5);
		...
		//Content touch end
		contentTouchEnd:function(e, anim) {
			...
			var speed = accDist/diff;
			var subtDist = Math.abs(this.content.slideLength-dist);
